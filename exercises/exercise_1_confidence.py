...
def node_analyze(state: ReviewState) -> dict:
        ...
        analysis = llm.invoke([
            {
                ...
                "content": (
                    ...
                    "and return the required structured review. Calibrate confidence "
                    "based on review completeness, risk, and uncertainty. Use these "
                    "bands: >=0.73 only for low-risk mechanical changes; 0.58-0.72 "
                    "for medium-risk small features, schema additions, validation, "
                    "testing, or documentation questions; <0.58 only for high-risk "
                    "security, auth, credential, injection, data-loss, or destructive "
                    "migration concerns."
                ),
            },
            {
    ...
    console.print(
        f"  [green]âœ“[/green] confidence={analysis.confidence:.0%}, "
...
def node_route(state: ReviewState) -> dict:
    ...
    console.print(
        f"  [green]âœ“[/green] decision=[bold]{decision}[/bold] "
