...
async def load_recent_sessions(limit: int = 10) -> list[dict]:
...
with st.sidebar:
    ...
    for row in sessions:
        label = f"{row['worst_risk'].upper()} · {row['thread_id'][:8]}"
        if st.button(label, key=f"session_{row['thread_id']}", use_container_width=True):
            ...
            st.session_state.final = None
            st.rerun()
        st.caption(f"{row['events']} events · {row['last_event']}")
...
def render_escalation_card(payload: dict) -> dict | None:
    ...
    with st.form("escalation"):
        ...
        for i, question in enumerate(payload.get("questions", []), start=1):
            st.markdown(f"**Q{i}. {question}**")
            answers[question] = st.text_area(
                "Answer",
                key=f"escalation_answer_{i}",
